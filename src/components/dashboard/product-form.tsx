...
export function ProductForm({
  ...
  const [stock, setStock] = useState(
    product && "stock" in product ? (product.stock as number) : 0,
  );
  ...
  const addVariant = () => {
    setVariants([
      ...variants,
      { sizeType: "", value: "", unit: "", price: "", stock: 0 },
    ]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const variant of variants) {
      if (!variant.sizeType || !variant.unit) {
        setError(
          "Please complete all variant fields or remove incomplete variants",
        );
        return;
      }
      if (variant.stock < 0) {
      ...
      if (variant.price) {
        ...
        if (isNaN(variantPrice) || variantPrice < 0) {
          setError(
            "Please enter a valid price for all variants or leave empty to use base price",
          );
          return;
        }
      ...
      if (isEditing && product) {
        await updateProduct(product.id, {
          ...
          variants: variants.map((v) => ({
            ...
            unit: v.unit as Unit,
            price: v.price
              ? Math.round(parseFloat(v.price) * 100) / 100
              : undefined,
            stock: v.stock,
          })),
        });
        ...
        if (
          result &&
          "error" in result &&
          result.error === "PRODUCT_LIMIT_REACHED"
        ) {
          setUpgradeInfo({
        ...
        // Track product added in GA4
        if (result && !("error" in result)) {
          import("@/lib/gtag").then(({ trackProductAdded }) => {
            trackProductAdded((result as { id: string }).id);
          });
        }
      }
        ...
        <div className="space-y-6">
          ...
          {categories && categories.length > 0 && (
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1.5">
                Products without a category will appear under &quot;Other&quot;
                on your storefront
              </p>
            </div>
          )}
        ...
        <div className="space-y-6 pt-8 border-t border-gray-200">
          <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Base price and inventory level
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-0 top-0 bottom-0 w-10 flex items-center justify-center text-gray-500 font-semibold text-lg">
                $
              </span>
              <input
                id="price"
